...
public class JsonParser {
    ...
    public Pagination getPagination(WebLinkCreator wlc) throws IOException {
        ...
        for(int i = 0; i < pageInfo.getLast(); i++){
            /*After the final test the .json file was more than 700mb, that`s why i have decided to separate this File
            * into 465, so they save the readable format*/
            FileWriter aboutYouJson = new FileWriter("AboutYou" + i + ".json");
            nextPage = wlc.getLink();
            ...
            aboutYouJson.close();
            result.put("totalAmountProducts", amountOfProducts);
        }
